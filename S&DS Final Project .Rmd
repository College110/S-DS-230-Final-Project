...
We find a value of 1.7621, which is less than 2 therefore demonstrating that the variances are indeed the same across professions. Next, we develop our ANOVA model, 'aov1' that 
...
This suggests a natural log transformation. So, we repeat the entire process we just did by creating a new column called 'transformed_gross' with a transformed value of domesticgross to the value found using boxCox above: 0.2222.
...
aov1 <- aov(movie1$transformed_gross ~ movie1$primary_profession) 
summary(aov1)
...
pairwise.t.test(movie1$transformed_gross, movie1$primary_profession)
